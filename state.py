...
# encoding: utf-8
r"""
Module containing all griddle State object
"""
...
class State(object):
    r"""
    A griddle State object contains the current state on a particular patch,
    ...
    from a Domain object:
        ...
        >>> import griddle
        >>> x = griddle.Dimension(0.0,1.0,100, name='x')
        >>> domain = griddle.Domain(x)
        >>> num_eqn = 1
        >>> solution = griddle.Solution(num_eqn,domain)
    ...
    def __init__(self,geom,num_eqn,num_aux=0):
        from griddle import geometry
        if isinstance(geom,geometry.Patch):
            self.patch = geom
        elif isinstance(geom,geometry.Domain):
            self.patch = geom.patches[0]
        else:
            raise Exception("""A State object must be initialized with
                             a Patch object.""")

        # ========== Attribute Definitions ===================================
        r"""griddle.Patch.patch - The patch this state lives on"""
        self.p   = None
    ...
    def is_valid(self):
        ...
        import logging
        valid = True
        logger = logging.getLogger('griddle.solution')
