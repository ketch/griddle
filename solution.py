...
# encoding: utf-8
r"""
Module containing all solution objects
"""
...
from griddle.geometry import Patch, Dimension, Domain
...
class Solution(object):
    ...
    :Initialization:
        ...
        The initialization of a Solution can happen one of these ways
            ...
            2. `args` is an integer (the number of components of q), a single
               ...
               by the appropriate :ref:`geometry <griddle_geometry>` object
               which can be one of:
                
                 - (:class:`~griddle.geometry.Domain`)
                 - (:class:`~griddle.geometry.Patch`) - A domain is created
                   with the patch or list of patches provided.
                 - (:class:`~griddle.geometry.Dimension`) - A domain and 
    ...
    :Examples:
        ...
        >>> import griddle
        >>> x = griddle.Dimension(0.,1.,100, name='x')
        >>> domain = griddle.Domain((x))
        >>> state = griddle.State(domain,3,2)
        >>> solution = griddle.Solution(state,domain)
    """
    ...
    def __init__(self,*arg,**kargs):
        ...
        if len(arg) == 1:
            ...
            if not isinstance(frame,int):
                raise Exception('Invalid griddle.Solution object initialization')
    ...
    def write(self,frame,path='./',file_format='ascii',file_prefix=None,
        ...
        for form in format_list:
            ...
            logging.getLogger('griddle.io').info(msg)
    ...
    def read(self, frame, path='./_output', file_format='ascii', 
        ...
        logging.getLogger('griddle.io').info("Read in solution for time t=%s" % self.t)

    def get_read_func(self, file_format):
        from griddle import io
